...
<div class="right">
       ...
       <div class="search">
       <form method="get" action="${pageContext.request.contextPath }/jsp/blist">
		   ...
		   <select id="providerSelect" name="queryProviderId">
			   <option value="0">--请选择--</option>
		   </select>
			...
			<select name="queryIsPayment">
			 ...
			 <a href="${pageContext.request.contextPath }/jsp/bredirectb">添加订单</a>
		</form>
       </div>
       <!--账单表格 样式和供应商公用-->
		...
		$.ajax({
			...
			"success":function (data) {
				for (var i = 0; i <data.length ; i++) {
					var provider=data[i];
					$("<option></option>").text(provider.proName).attr("value",provider.id).appendTo($("#providerSelect"));
				}
			}
		});
	})
